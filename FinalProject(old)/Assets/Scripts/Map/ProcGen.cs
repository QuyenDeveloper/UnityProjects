...
sealed class ProcGen : MonoBehaviour
{
    public void GenerateDungeon(int mapWidth, int mapHeight,int roomMaxSize, int roomMinSize, int maxRooms, int maxMonstersPerRoom, List<RectangularRoom> rooms)
    {
        for (int roomNum = 0; roomNum < maxRooms; roomNum++)
        {
            ...
            if(rooms.Count != 0)
            {
                TunnelBetween(MapManager.instance.Rooms[rooms.Count - 1], newRoom);
            }
            else
            {
    ...
    private void PlaceActors(RectangularRoom newRoom, int maximunMonsters)
    {
        int numberOfMonsters = Random.Range(0, maximunMonsters + 1);
